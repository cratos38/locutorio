...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      .from('users')
      .select('id, username, email, nombre, edad, genero, ciudad, created_at, is_admin, is_verified, is_plus, presence_status, last_login', { count: 'exact' });
    ...
    switch (status) {
      case 'active':
        // query = query.eq('is_banned', false);  // Campo no existe
        break;
      case 'banned':
        // query = query.eq('is_banned', true);   // Campo no existe
        break;
      case 'unverified':
        query = query.eq('is_verified', false);  // Usar is_verified en lugar de email_verified
        break;
    }
