#!/usr/bin/env python3
"""
ML Photo Validator v3.1 - Smart Crop, Face-Only Sharpness + NSFW Detection
===========================================================================
...
- ✅ Detección NSFW con NudeNet (nuevo en v3.1)
...
# ============================================================================
# CARGAR MODELO NSFW AL INICIO
# ============================================================================
print("🔍 Cargando modelo NSFW...")
from nudenet import NudeDetector
nsfw_detector = NudeDetector()
print("✅ Modelo NSFW cargado")
...
VALIDATION_CONFIG = {
    'profile': {
        ...
        'nsfw_enabled': True                # Validar NSFW
    },
    'album': {
...
def check_nsfw(img_array):
    """
    Detecta contenido NSFW en una imagen usando NudeNet
    
    Args:
        img_array: numpy array RGB de la imagen
    
    Returns:
        dict con:
        - is_nsfw: bool (True si hay contenido explícito que debe rechazarse)
        - verdict: 'explicit', 'questionable', 'safe'
        - confidence: score más alto detectado
        - explicit_detections: lista de partes explícitas detectadas
        - questionable_detections: lista de partes cuestionables
    """
    ...
    try:
        # Analizar con NudeNet
        detections = nsfw_detector.detect(tmp.name)
        
        # Clasificar detecciones según nivel de exposición
        explicit_classes = [
        ...
        questionable_classes = [
            ...
            'FEET_EXPOSED',
            'ARMPITS_EXPOSED',
            'FEMALE_BREAST_COVERED'  # Bikini, ropa ajustada, etc.
        ]
        ...
        for det in detections:
            ...
            # Contenido cuestionable (umbral 70%)
        ...
        if explicit_detections:
            verdict = 'explicit'
            is_nsfw = True
        elif len(questionable_detections) >= 2:  # 2+ partes cuestionables
            verdict = 'questionable'
            is_nsfw = False  # No rechazar automáticamente, enviar a revisión manual
        else:
            verdict = 'safe'
            is_nsfw = False
...
def validate_and_crop_photo(photo_url, photo_type='profile', user_gender=None, user_age=None):
    ...
    3. Detectar rostros
    4. Validar NSFW (nuevo)
    5. Calcular % de rostro (con imagen redimensionada)
    6. Crop inteligente (márgenes generosos)
    7. Calcular nitidez SOLO en zona del rostro
    8. Validar resolución del crop
    9. Devolver resultado + cropped image
    ...
    try:
        ...
        # =====================================================================
        # 4. VALIDACIÓN NSFW (NUEVO EN v3.1)
        # =====================================================================
        if config.get('nsfw_enabled', True):
            ...
            print(f"   Detecciones explícitas: {len(nsfw_result['explicit_detections'])}")
            print(f"   Detecciones cuestionables: {len(nsfw_result['questionable_detections'])}")
            
            # Rechazar si hay contenido explícito
            if nsfw_result['is_nsfw']:
                explicit_parts = [d['part'] for d in nsfw_result['explicit_detections']]
                print(f"❌ RECHAZADA: Contenido explícito detectado ({', '.join(explicit_parts)})")
                return {
                    "verdict": "REJECT",
                    "reason": "nsfw_content",
                    "message": f"Contenido inapropiado detectado: {', '.join(explicit_parts)}",
                    "nsfw_data": {
                        "verdict": nsfw_result['verdict'],
                        "confidence": nsfw_result['confidence'],
                        "detections": nsfw_result['explicit_detections']
                    },
                    "processing_time": round(time.time() - start_time, 2)
                }
            
            # Marcar para revisión manual si es cuestionable
            if nsfw_result['verdict'] == 'questionable':
                questionable_parts = [d['part'] for d in nsfw_result['questionable_detections']]
                print(f"⚠️ CONTENIDO CUESTIONABLE: {', '.join(questionable_parts)} - continuará a revisión manual")
        ...
        # =====================================================================
        # 8. CALCULAR NITIDEZ SOLO EN ZONA DEL ROSTRO
        # =====================================================================
        print("\n🔎 ANALIZANDO NITIDEZ:")
        ...
        # =====================================================================
        # 9. REDIMENSIONAR CROP AL TAMAÑO OBJETIVO
        # =====================================================================
        ...
        # =====================================================================
        # 11. DETERMINAR VEREDICTO FINAL
        # =====================================================================
        processing_time = round(time.time() - start_time, 2)
        
        # Verificar si hay contenido cuestionable NSFW
        has_questionable_nsfw = (config.get('nsfw_enabled', True) and 
                                 nsfw_result.get('verdict') == 'questionable')
        
        # Si nitidez, % rostro o NSFW están en zona de revisión manual
        if (sharpness < config['min_sharpness'] or 
            face_percentage < config['min_face_percentage'] or
            has_questionable_nsfw):
            ...
            if has_questionable_nsfw:
                questionable_parts = [d['part'] for d in nsfw_result['questionable_detections']]
                reasons.append(f"contenido cuestionable ({', '.join(questionable_parts)})")
            ...
            # Agregar datos NSFW si está habilitado
            if config.get('nsfw_enabled', True):
                result['nsfw_data'] = {
                    'verdict': nsfw_result['verdict'],
                    'confidence': nsfw_result['confidence'],
                    'questionable_detections': nsfw_result['questionable_detections']
                }
...
if __name__ == '__main__':
    print("\n" + "="*70)
    print("🚀 ML VALIDATOR v3.1 - SMART CROP, FACE-SHARPNESS + NSFW")
    print("="*70)
    print("\n📍 ENDPOINTS:")
    ...
    print("\n⚙️ CONFIGURACIÓN:")
    print("   • Profile:  min_face=10%, min_sharpness=50, crop_margin=80%, NSFW=ON")
    print("   • Album:    min_face=5%,  min_sharpness=40, crop_margin=50%, NSFW=ON")
    ...
    print("   ✅ Detección NSFW con NudeNet (NUEVO)")
    print("="*70 + "\n")
